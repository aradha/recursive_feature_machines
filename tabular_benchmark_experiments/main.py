...
print ("Dataset\tSize\tNumFeatures\tNumClasses\tValidation Acc\tReg\tIter\tTest Acc", file = outf)
...
for idx, dataset in enumerate(sorted(os.listdir(datadir))):
    ...
    best_acc, best_reg, best_iter, best_M = 0, 0, 0, 0
    ...
    for reg in regs:
        for n in normalize:
            if dataset == 'balance-scale':
                n = False
            acc, iter_v, M = rfm.hyperparam_train(X[train_fold], y[train_fold],
                                                  X[val_fold], y[val_fold], c,
                                                  iters=max_iter, reg=reg, normalize=n)
            if acc > best_acc:
                best_acc = acc
                best_reg = reg
                best_iter = iter_v
    ...
    for repeat in range(4):
        ...
        acc = rfm.train(X[train_fold], y[train_fold], X[test_fold], y[test_fold],
                         c, best_M, iters=best_iter, reg=best_reg, normalize=best_normalize)
        avg_acc += 0.25 * acc

    print ("acc:", avg_acc, best_reg, best_iter, best_normalize,"\n")
    print(str(dataset) + '\t' + str(n_tot) + '\t' + str(d) + '\t' + str(c) + '\t' + str(best_acc) + '\t' + str(best_reg) + \

...
def get_data(dataset, mask=None, num_samples=-1,
             break_idx=0):
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        if m == 1 and g != 1:
            ...
            #template = ex * mask
            # Uncomment to use Lips mask
            #template = ex[:, 63:75, 30:65]
    ...
    for idx in tqdm(range(len(dataset))):
        ...
        # Uncomment for lips mask
        #ex[:, 63:75, 30:65] = template
...
def main():
    ...
    path = 'path to lipstick nn model'
    d = torch.load(path)
    ...
    M = M.T @ M
    M_path = 'path to RFM lipstick model M'
    ...
    # Writing to log file
    f = open('csv_logs/lipstick_log.csv', 'w')
